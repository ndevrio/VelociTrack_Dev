...
a3.setYRange(0.5, -7, padding=0.1)
...
depth = np.zeros((len(raw_acc_buffer),))
last_time = 0
def process_data():
    global last_time, raw_acc_buffer, kill_threads, accel, vel, depth
    ...
    while not kill_threads:
        ...
        if(len(acc) == 3):
            ...
            # Convert data
            accel = -1*(raw_acc_buffer[:, 2] - 1.11)

            dt = 1/1000
            vel = np.roll(vel, 1)
            vel[0] = vel[1] + accel[0] * dt
            
            depth = np.roll(depth, 1)
            depth[0] = depth[1] + vel[0] * dt
