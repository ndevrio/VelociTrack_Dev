...
data[:, -5:-2] = np.roll(data[:, -5:-2], 5, axis=0)
...
if not os.path.exists((sys.argv[1] + "_" + sys.argv[2] + "_stat_features.npy")):
    ...
    for i in tqdm(range(int(feature_window_size/2), len(data)-int(feature_window_size/2))):
        window = data[i-int(feature_window_size/2):i+int(feature_window_size/2), -5:-2]
        ...
        # [0] Maximum
        stat_features[i, 0:3] = np.max(window, axis=0)
        # [1] Minimum
        stat_features[i, 3:6] = np.min(window, axis=0)
        # [2] Mean
