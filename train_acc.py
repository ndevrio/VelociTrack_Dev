...
from mmapdataset import AccDataset
...
def main():
    ...
    train_dataset = AccDataset(sys.argv[1], sys.argv[2], sys.argv[3], 0)
    test_dataset = AccDataset(sys.argv[1], sys.argv[2], sys.argv[3], 1)

    model = Acc1DCNN()
    data = TimeSeriesData(train_dataset, test_dataset)
    log_time = time.strftime("%Y%m%d-%H%M%S")
    log_name = "imu_" + sys.argv[2] + "_" + sys.argv[3] # camera
