...
def invariant_match_template(dispframe, rgbimage, rgbtemplate, method, matched_thresh, rot_range, rot_interval, scale_range, scale_interval):
    """
    ...
    # #print(">>> ", all_points[0][0])#, np.mean(all_points[:3][0]))
    # # x = (all_points[0][0][0] + all_points[1][0][0] + all_points[2][0][0])/3.0
    # # y = (all_points[0][0][1] + all_points[1][0][1] + all_points[2][0][1])/3.0
    # # all_points[0][0] = (x, y)
    # # print((x, y))
...
class CameraCaptureThread(threading.Thread):
    def __init__(self, batch_size=5, dev_info=None):
        ...
        mvsdk.CameraSetExposureTime(self.hCamera, 1200)
...
class App(object):
    def __init__(self):
        ...
        self.REINIT_LANDMARKS_COUNT = 40 #80#2400 #180 # Can change this higher to get faster FPS at the expense of worse tracking
        ...
        self.mbbox = (0, 0, self.TEMPLATE_SIZE, self.TEMPLATE_SIZE)
        self.template = None
        self.region = None
    ...
    def main(self):
        ...
        dispframe = np.zeros((480, 1195, 3), dtype=np.uint8)

        while not self.quit:
            try:
                # get items in queue
                # print("Loop: ", camera_capture_thread.get_number_in_queue())
                if camera_capture_thread.get_number_in_queue() == 0:
                    ...
                    camera_frame_count = camera_capture_thread.frame_count
                #### Frame processing
                ...
                # print("receive frames", 1000 * (b - a), "ms")   
                ...
                if self.previous_index_finger_tip == [] or self.s_num_frames % self.REINIT_LANDMARKS_COUNT == 0:
                    ...
                    ### If a tracking result is found
                        ...
                        # print(self.previous_index_finger_tip_mp[0])
                        previous_bbox = np.array([(self.previous_index_finger_tip_mp[0] - self.TEMPLATE_SIZE / 2), (self.previous_index_finger_tip_mp[1] - self.TEMPLATE_SIZE / 2), (self.TEMPLATE_SIZE), (self.TEMPLATE_SIZE)])#, dtype=np.int16)

                        self.previous_index_finger_tip = self.previous_index_finger_tip_mp
                        self.bbox = previous_bbox#.astype('int16')
                        self.mbbox = previous_bbox
                ...
                ############################################################
                    ...
                    self.region = bbox_to_sobel(sframe, region_bbox)
                    
                    # Perform template matching
                    try:
                        invariant_result = invariant_match_template(dispframe, rgbimage=self.region, rgbtemplate=self.template, method="TM_CCOEFF_NORMED", matched_thresh=0.5, rot_range=[-10,20], rot_interval=10, scale_range=[90,110], scale_interval=10)
                        # invariant_result = invariant_match_template(rgbimage=region, rgbtemplate=self.template, method="TM_CCOEFF_NORMED", matched_thresh=0.2, rot_range=[-20,21], rot_interval=10, scale_range=[90,111], scale_interval=10)
                        ...
                        if(play_mode == 0):
                            # sound_p = multiprocessing.Process(target=playsound, args=(sound_file_path,))
                            # sound_p.start()
                ...
                if self.previous_index_finger_tip != []:
                    ...
                    ### MediaPipe reset smoothing
                    if(self.mp_did_reset):
                        self.reset_counter = 5
                    if(self.reset_counter > 1):
                        # tip_x = int((self.last_tip_x * (self.reset_counter - 1) + tip_x) / self.reset_counter)
                        # tip_y = int((self.last_tip_y * (self.reset_counter - 1) + tip_y) / self.reset_counter)
                        # print(self.reset_counter, ": ", tip_x, tip_y)
                        self.reset_counter -= 1
                
                    #dst_tip_x, dst_tip_y = remap_fingertip_pixel(tip_x, tip_y)
                        
                    # print("IN: ", tip_x, tip_y, "     OUT: ", dst_tip_x, dst_tip_y)
                    
                    ### Send x and y and whether the hand was reinitialized and timestamp
                    message = f"{self.frame_timestamp},{int(tip_x)},{int(tip_y)},{self.mp_did_reset},{self.hand_tracked}" # Mediapipe tracking, template match found
                ...
                if (self.DEBUG and self.s_num_frames % 30 == 0):
                    try:
                        ...
                        # if self.previous_index_finger_tip != []:
                        # sframe[int(self.bbox[1]):int(self.bbox[1])+int(self.bbox[3]), int(self.bbox[0]):int(self.bbox[0])+int(self.bbox[2]), 0] = sobel_frame
                        # sframe[int(self.bbox[1]):int(self.bbox[1])+int(self.bbox[3]), int(self.bbox[0]):int(self.bbox[0])+int(self.bbox[2])] = sobel_frame

                        # cv2.circle(sframe, (int(self.last_tip_y), int(self.last_tip_x)), 10, (255, 0, 255), 1)

                        try:
                            # draw the previous fingertip
                            sframe = cv2.rotate(sframe, cv2.ROTATE_180)
                            sframe = cv2.putText(sframe,(str(self.fps+80) + " FPS"), (15, 45), cv2.FONT_HERSHEY_SIMPLEX, 1.25, (255,255,255), 2, 2)

                            dispframe[:, :640] = np.expand_dims(sframe, 2)
                            
                            cv2.rectangle(dispframe, (640-int(self.mbbox[0]), 480-int(self.mbbox[1])), (640-int(self.mbbox[0])-int(self.mbbox[2]), 480-int(self.mbbox[1])-int(self.mbbox[3])), (255, 0, 255), 1)
                            cv2.circle(dispframe, (640-int(self.previous_index_finger_tip[0]), 480-int(self.previous_index_finger_tip[1])), 10, (0, 255, 0), 1)

                            if(self.template is not None and self.region is not None):
                                self.region = cv2.GaussianBlur(self.region, (9,9), 0)
                                scaled_template_gray, _ = scale_image(self.template, 600.0)
                                scaled_img_gray, _ = scale_image(self.region, 600.0)

                                dispframe[:scaled_template_gray.shape[0], 645:645+scaled_template_gray.shape[1]] = np.expand_dims(np.rot90(scaled_template_gray, k=2), 2)
                                dispframe[:scaled_img_gray.shape[0], 890:890+scaled_img_gray.shape[1]] = np.expand_dims(np.rot90(scaled_img_gray, k=2), 2)


                            cv2.imshow("img", dispframe)
...
def bbox_to_sobel(frame, region_bbox):
    ...
    #output = clahe.apply(frame_box)
    
    # Apply Sobel operator in x and y directions
    # try:
    #     sobel_x = cv2.Sobel(frame_box, cv2.CV_32F, 1, 0, ksize=9)
    #     sobel_y = cv2.Sobel(frame_box, cv2.CV_32F, 0, 1, ksize=9)

    #     # Calculate the gradient magnitude
    #     magnitude = cv2.sqrt(sobel_x**2 + sobel_y**2)

    #     # Normalize the magnitude for better visualization
    #     output = cv2.normalize(magnitude, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
    #     output[output < 150] = 0
    # except cv2.error:
    #     pass
    ...
    output = res.reshape((image.shape))
