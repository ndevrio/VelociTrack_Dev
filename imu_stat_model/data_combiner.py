...
import itertools
from funcs_stat import combine_data
...
for i in tqdm(range(len(comb_list))):
    if(comb_list[i][0] == "p1" and comb_list[i][1] < 7):
        continue
    # print(comb_list)
    # print(comb_list[i][0], str(comb_list[i][1]))
    acc_win, gt = combine_data(comb_list[i][0], str(comb_list[i][1]), ds)
    ...
    ######################################################################
    window_size = int(40/ds) # used to calculate overlap (320 / 800 = 400 ms)
    m = int(window_size / 10) # 10% overlap
    num_windows = int((labels_win.shape[0] - window_size - 1) / m)
    labels = np.zeros((num_windows, window_size))
    acc = np.zeros((num_windows, window_size, 4))


    def preprocessing():
        global labels, acc

        # [1] Split continuous data into windows with 10% overlap
    ...
    labels_fp = np.memmap(outfile + "_stat_labels" + ".bin", dtype='ubyte', mode='w+', shape=labels.shape)
    labels_fp[:] = labels[:]
    labels_fp.flush()
    print('Wrote\t', outfile + "_stat_labels" + ".bin\t", labels_fp.shape)

    acc_fp = np.memmap(outfile + "_stat_acc" + ".bin", dtype='float32', mode='w+', shape=acc.shape)
    acc_fp[:] = acc[:]
    acc_fp.flush()
    print('Wrote\t', outfile + "_stat_acc" + ".bin\t", acc_fp.shape)
